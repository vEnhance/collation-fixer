...
def set_logging_level(level: Union[str, int] = None, logger="colfixer"):
    ...
    if empty(level):
        level = (
            "ERROR"
            if settings.QUIET
            else env("LOG_LEVEL", ("DEBUG" if settings.DEBUG else "WARNING"))
        )
...
def query(
    stmt, *params, one=False, use_tx=True, **kwargs
) -> Optional[Union[Tuple[Any, ...], str, int, float, bool, Decimal]]:
    conn = connect()
    if use_tx:
        conn.begin()
    ...
    try:
        ...
        log.exception(
            "Exception while executing query: '%s' - params: %s", stmt, list(params)
        )
        if use_tx:
            conn.rollback()
        raise e
...
def convert_table(
    table: str, charset="utf8mb4", collation="utf8mb4_unicode_ci", use_tx=True
):
...
def convert_tables(
    *tables: str, charset="utf8mb4", collation="utf8mb4_unicode_ci", use_tx=True
):
    conn = connect()
    if use_tx:
        conn.begin()
    ...
    for tb in tables:
        try:
            log.info(
                "Converting table %s to charset %s and collation %s - use_tx: %s",
                tb,
                charset,
                collation,
                use_tx,
            ...
            res = convert_table(
                tb, charset=charset, collation=collation, use_tx=not use_tx
            )
            results += [(tb, res)]
        except Exception as e:
            if use_tx:
                ...
                raise e
            log.warning(
                "Exception while converting table %s to %s %s - ignoring error and moving on.",
                tb,
                charset,
                collation,
            )
...
def get_column(
    table: str, column: str, database=None, fail=False
) -> Optional[TableColumnResult]:
...
def convert_column(
    table: str, column: str, charset="utf8mb4", collation="utf8mb4_unicode_ci", **kwargs
):
    ...
    stmt = (
        f"ALTER TABLE {table} MODIFY {col.column} {col.column_type} "
    ...
    **kwargs,
    ...
    if all([empty(database), empty(settings.DB_NAME)]):
        raise AttributeError(
            "No database specified in args, nor in settings.DB_NAME - cannot continue!"
        )

    if all([empty(columns, itr=True), not conv_all]):
        raise AttributeError(
            "No columns specified in args, and conv_fall is False - cannot continue!"
        )
    ...
    for c in cols:
        try:
            if not conv_all and c.column not in columns:
                log.info(
                    "Skipping column '%s' on table '%s' - not in columns arg.",
                    c.column,
                    table,
                )
                continue
            if not empty(c.column_key) and skip_indexed:
                log.info(
                    "Skipping column '%s' on table '%s' - column is an index!",
                    c.column,
                    table,
                )
                continue
            if empty(c.character_set) and empty(c.collation):
                log.info(
                    "Skipping column '%s' on table '%s' - column doesn't support char sets!",
                    c.column,
                    table,
                )
                continue
            if (
                c.character_set.lower() == charset.lower()
                and c.collation.lower() == collation.lower()
            ):
                log.info(
                    "Skipping column '%s' on table '%s' - column is already collation '%s' and charset '%s'",
                    c.column,
                    table,
                    collation,
                    charset,
                ...
                continue
            log.info(
                "Converting column '%s' on table '%s' to charset %s and collation %s",
                c.column,
                table,
                charset,
                collation,
            ...
            if use_tx:
                ...
                raise e
            log.warning(
                "Exception while converting column %s to %s %s - ignoring error and moving on.",
                c.column,
                charset,
                collation,
            )
