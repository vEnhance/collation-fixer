...
def columnize(*cols: str, size: int = 40) -> str:
    ...
    cols = [
        str(c)
        if isinstance(c, (int, float, Decimal)) or c in [None, False, True]
        else c
        for c in cols
    ]
...
def convert_tables(opts):
    ...
    charset, collation = (
        empty_if(opts.charset, "utf8mb4", itr=True),
        empty_if(opts.collation, "utf8mb4_unicode_ci", itr=True),
    )
    # table = empty_if(opts.table, None, itr=True)
    ...
    if all_tables:
        ...
        print(YELLOW)
        print(
            f" >>> --all-tables was specified. Converting {len(tables)} tables! The tables are: {', '.join(tnames)}"
        )
        print(RESET)
    else:
...
def _convert_columns(
    ...
    **kwargs,
    ...
    tnames = [t.table for t in tables]
    print(
        f"{YELLOW} >>> Converting columns in {len(tables)} tables. Tables are: {', '.join(tnames)}{RESET}\n"
    )
    for t in tables:
        print(
            f"{CYAN}    [-] Converting columns in table {t.table} to charset {charset} and collation {collation}{RESET}"
        )
        try:
            ...
            print(
                f"{GREEN}    [+] Finished converting columns in table {t.table}{RESET}\n"
            )

        except Exception as e:
            log.exception(
                "Error while converting columns in table %s - %s - %s",
                t.table,
                type(e),
                str(e),
            )
...
mparser.add_argument(
    ...
    help=f"Hostname/IP of database server. Default: {settings.DB_HOST}",
)
mparser.add_argument(
    "-u",
    "--user",
    default=settings.DB_USER,
    help=f"Username to login to database server. Default: {settings.DB_USER}",
)
mparser.add_argument(
    "-p",
    "--pass",
    dest="password",
    default=settings.DB_PASS,
...
parse_ct.add_argument(
    "--db", default=None, help="MySQL database to use instead of DB_NAME", nargs="?"
)
parse_ct.add_argument(
    "-a",
    "--all-tables",
...
parse_ct.add_argument(
    "--charset",
    default="utf8mb4",
    help="Character set to convert columns to (default: utf8mb4)",
)
parse_ct.add_argument(
    "--collation",
    default="utf8mb4_unicode_ci",
    help="Collation to convert columns to (default: utf8mb4_unicode_ci)",
)
parse_ct.add_argument(
    "--no-tx",
    dest="outer_tx",
...
parse_ct.set_defaults(
    func=convert_tables, all_tables=False, outer_tx=True, skip_indexed=True
)
...
parse_cc.add_argument(
    "-c",
    "--columns",
    dest="columns",
    default=[],
    help="Table columns to convert",
    nargs="*",
)
parse_cc.add_argument(
    "--db", default=None, help="MySQL database to use instead of DB_NAME"
)
parse_cc.add_argument(
    "--charset",
    default="utf8mb4",
